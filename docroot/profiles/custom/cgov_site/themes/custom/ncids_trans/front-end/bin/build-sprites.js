...
const spritePath = path.join(__dirname, '../../../cgov/cgov_common/assets/sprites/*.svg');
const spriteConfig = {
  shape: {
    id: {
      separator: ''
  ...
  mode: {
    view: {
      ...
      render: {
        scss: {
          dest: path.join(__dirname, '../src/styles/sprites/_svg-sprite-map.scss'),
          template: path.join(__dirname, '../../../cgov/src/styles/sprites/sprite-template.scss'),
        },
      },
      layout: 'packed',
      bust: true,
...
const addFileToSpriter = (spriteFile, fullPath, spriter) => {
  ...
  const basename = basepos >= 0 ? spriteFile.substr(basepos + 2) : path.basename(spriteFile);
