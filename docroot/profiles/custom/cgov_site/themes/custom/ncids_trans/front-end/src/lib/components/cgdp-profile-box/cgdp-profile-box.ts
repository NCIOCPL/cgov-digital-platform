 ...
 * @param linkText Text content of the link clicked
 */
const track = (
...
const linkClickHandler: EventListener = (evt: Event) => {
	...
	const linkText = (target as HTMLAnchorElement).textContent?.trim() || '';

	track(profileBoxType, profileBoxTitle, profileField, linkType, linkText);
};
