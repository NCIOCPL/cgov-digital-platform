...
/*
  The map topology collection is versioned separately from Highcharts core and modules:
...
/**
 * Initialize the grants/contracts map and its institution-detail popups.
 */
function initChart(Chart, data, miscData = {}) {
  ...
  // Store floating popups so resize and repeated state clicks can reuse them.
  var popups = [];
  var popupById = {};

  /**
   * Keep a floating popup fully visible after opening, dragging, or resizing.
   */
  ...
  /**
   * Re-clamp open popups when the browser viewport changes.
   */
  ...
  /**
   * Stagger multiple desktop popups while keeping mobile popups centered.
   */
  ...
  /**
   * Raise a popup above the other open popups.
   */
  ...
  /**
   * Center a popup in the viewport and then apply the staggered offset.
   */
  ...
  /**
   * Add titlebar dragging without bringing in jQuery UI.
   */
    ...
    titlebar.addEventListener('pointerdown', function (event) {
      ...
      function drag(event) {
        ...
        popup.element.style.left =
          Math.floor(startLeft + event.clientX - startX) + 'px';
        popup.element.style.top =
          Math.floor(startTop + event.clientY - startY) + 'px';
        keepPopupInViewport(popup);
      }
      ...
      dragTarget.addEventListener('pointermove', drag);
  ...
  /**
   * Reflow the nested Highcharts pie when its popup is resized.
   */
  ...
  /**
   * Create a lightweight dialog shell that preserves legacy jQuery UI class hooks.
   */
  function createPopup(options) {
    ...
    popupElement.className =
      'ui-dialog ui-corner-all ui-widget ui-widget-content ui-front cgdp-highcharts-dialog';
    ...
    popupElement.style.minWidth = options.minWidth + 'px';
    ...
    // Expose only the popup operations this chart needs.
    var popup = {
  ...
  /**
   * Build the map series and attach state-click behavior.
   */
    ...
    return [
      {
        ...
        point: {
          events: {
            click: function () {
              // States with institution data open a reusable pie-chart popup.
              if (this.options.institutions) {
                /**
                 * Render an institution breakdown pie chart inside a state popup.
                 */
                      ...
                      pie: {
                        ...
                        point: {
                          events: {
                            legendItemClick: function () {
                              // Preserve popup drilldown behavior by disabling legend toggles.
                              return false;
                            },
                          },
                        },
                        events: {
                          mouseOver: function () {
                            ...
                            Object.keys(legend.allItems).forEach(
                              function (key) {
                    ...
                    tooltip: {
                      ...
                      formatter: function () {
                        ...
                        var template =
                          ...
                          Highcharts.numberFormat(grants, 0) +
                          '</b></td></tr><tr><td>Contracts:</td><td style="text-align:center"><b>$' +
                          Highcharts.numberFormat(contracts, 0) +
                ...
                if (popup) {
                  ...
                  // First click for this state: create the popup and chart.
                  popup = createPopup({
                    id: modalId,
                    title: this.name,
                    minWidth: 400,
                  ...
                  // Keep the institution pie legend from acting like a drilldown.
                  ...
                  popup.setChart(
                    renderPieChart(this.options, popup.content, isInteractive)
                  );
                }
              } else {
                // States without institution data share one reusable message popup.
                ...
                if (popup) {
                  ...
                  var message =
                    'This state/territory does not have any individual university or center receiving more than $15 million in NCI support.';
  ...
  /**
   * Build the main map tooltip showing grants, contracts, and total support.
   */
  function buildTooltip() {
    return {
      ...
      formatter: function () {
        var header =
          '<div><div style="font-size: 13px;margin-bottom:3px">' +
          this.point.options.state +
          '</div><table style="border-collapse:collapse;margin: 0 auto">';
        var template =
          '<tr><td>' +
          tooltipObj.label.grants +
          ' (' +
          this.point.grants.number +
          '): </td><td style="text-align: right;"><b>$' +
          Highcharts.numberFormat(this.point.grants.amount, 0) +
          '</b></td></tr><tr><td>' +
          tooltipObj.label.contracts +
          ' (' +
          this.point.contracts.number +
          '): </td><td style="text-align: right;"><b>$' +
          Highcharts.numberFormat(this.point.contracts.amount, 0) +
          '</b></td></tr><tr><td style="border-top: 1px solid #000;">' +
          this.series.name +
  ...
  // Normalize state codes for Highcharts map joins and derive color-axis values.
  allocationData.forEach(function (allocation) {
    allocation.code = allocation.code.toUpperCase();
    // Logarithmic color axes require positive values.
    allocation.value =
      allocation.grants.amount + allocation.contracts.amount || 0.00001;
  });
