...
export default class ProactiveLiveHelp {
  ...
  displayPrompt() {
    ...
    const secondsSinceLastInteraction = getSecondsSinceLastInteraction(
      this.userActivity.lastActivityTime
    );
    ...
    modalContainer.appendChild(popupElement);
    ...
    document
      ...
      .addEventListener("click", popupLiveHelpHandler);
  ...
  setUserToOptedOut() {
    CookieManager.set(this.options.popupID + "-opt", "true", {
      expires: this.options.optOutDurationDays,
    });
  }
  ...
  initializeCountdownTimer() {
    ...
    var tick =
      (timeleft >= this.options.timerIntervalSeconds
        ? this.options.timerIntervalSeconds
        : timeleft) * 1000;

    this.countdownIntervalID = window.setInterval(
      this.decrementCountdownTimer.bind(this),
      tick
    );
  }
...
function activatePromptAnalytics() {
  ...
  if (NCIAnalytics && NCIAnalytics.RecordProactiveChatPromptDisplay)
    NCIAnalytics.RecordProactiveChatPromptDisplay(
