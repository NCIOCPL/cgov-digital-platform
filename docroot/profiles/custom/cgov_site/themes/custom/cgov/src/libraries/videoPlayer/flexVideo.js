import { checkNodeAncestryForClass, getNodeArray, keyHandler } from 'Utilities';
...
const injectIFrame = (videoHook, videoSource) => {
  fetch(videoSource)
    ...
    .catch((err) => {
      console.error('Failed to load video embed', err);
    });
};
...
const renderPreviewThumbnails = () => {
  ...
  const filteredHooks = videoHooks.filter(
    (hook) => !checkNodeAncestryForClass(hook, 'yt-carousel')
  );
