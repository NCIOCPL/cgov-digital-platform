...
class CgovNavigationManager {
  ...
  protected function getInitialMenuItemId(TermInterface $root_term, $menu_type, $max_depth) {
    ...
    // root, so we can just return the id of the root.
    if ($root_idx === 0) {
      return $root_term->id();
    }

    // Full Ancestry is bottom up, but we want top down. So we need to slice
    // and flip. 0 will be the first child of the root.
    $branch_arr = array_reverse(array_slice($this->fullAncestry, 0, $root_idx));

    $deepest_node = $root_term;
    foreach ($branch_arr as $level => $term) {

      // If this item is past the max depth then we need to stop.
      // Remember, this loop starts with the children and depth counting
      // is 1-based, so the level of the first child (index 0) is 2.
      if ($level + 2 > $max_depth) {
        break;
      }

      // If this item is hidden in the nav, we need to stop.
